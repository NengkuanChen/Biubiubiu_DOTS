...
public partial class FirstPersonPlayerInputsSystem : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        foreach (var (playerCommands, player, ghostOwner, entity) in SystemAPI.Query<RefRW<FirstPersonPlayerCommands>, RefRW<FirstPersonPlayer>, GhostOwnerComponent>().WithEntityAccess())
        {
            ...
            // Crouch
            playerCommands.ValueRW.CrouchHeld = defaultActionsMap.Crouch.IsPressed();
            
            
            // Reload
            if (defaultActionsMap.Reload.WasPressedThisFrame())
            {
                playerCommands.ValueRW.ReloadPressed.Set();
            }
