...
namespace Lobby
{
    ...
    public partial struct PlayerChangePositionSystemServer: ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (req, reqEntity) in SystemAPI.Query<RefRO<ReceiveRpcCommandRequest>>().WithAll<PositionChangeRequestComponent>().WithEntityAccess())
            {
                var idComponent = state.GetComponentLookup<NetworkId>(true)[req.ValueRO.SourceConnection];
    ...
    public partial struct ClearPositionClient : ISystem
    {
        ...
        {
            var commandBuffer = new EntityCommandBuffer(Allocator.Temp);
            foreach (var (req, reqEntity) in SystemAPI.Query<RefRO<ClearOriginalButtonRequest>>().WithAll<ReceiveRpcCommandRequest>().WithEntityAccess())
            {
